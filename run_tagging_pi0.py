...
def run(filename, config, ismc, suffix=""):
    ...
    if config["common"]["do_ptspectrum"] == True:
        outname = "tagging_{0}_{1}_ptspectrum_{2}_{3}TeV_{4}{5}.root".format(particle, args.type, config["common"]["system"], config["common"]["energy"], config["common"]["period"], suffix);
        print("output file name = ",outname);
        outfile = TFile(outname,"RECREATE");
        if ismc:
            ana_pi0 = PairAnalyzer(particle, filename, "tagging-pi0-mc", ismc);
            ...
            for isys in range(0,nsys):
                ...
                ana_pi0.set_xtitle("#it{m}_{ee#gamma} (GeV/#it{c}^{2})");
                ana_pi0.set_ytitle("#it{p}_{T,#gamma} (GeV/#it{c}^{2})");
                        ...
                        for ibkg in list_fit_bkg:
                            ...
                            for ir in range(0, len(list_fit_min)):
                                ...
                                for iint in range(0, len(list_integral_min)):
                                    ...
                                    outlist_int_range = ana_pi0.analyze_ptspectrum_efficiency(True);
            ...
            ana_pi0 = PairAnalyzer(particle, filename, "tagging-pi0", ismc);
            ana_pi0.set_arr_pt(arr_pt);
            for isys in range(0,nsys):
                ssname = config[args.type]['subsystems'][isys]['name']; #subsystem name
                ana_pi0.set_subsystem(ssname);
