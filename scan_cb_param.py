...
from ROOT import TFile, THashList, TF1, TH2F
...
def run(filename, config, ismc, suffix=""):
    ...
    if config["common"]["do_ptspectrum"] == True:
        outname = "{0}_{1}_cb_param_{2}_{3}TeV_{4}{5}.root".format(particle, args.type, config["common"]["system"], config["common"]["energy"], config["common"]["period"], suffix);
                            ...
                            for ir in range(0, 1):
                                ...
                                for iint in range(0, 1):
                                    ...
                                    for ipt in range(0, len(arr_pt)-1):
                                        h2cb = TH2F("h2cb_pt{0:d}".format(ipt),"cb param;N;#alpha",20,0.05,2.05, 100,0.25, 50.25);
                                        list_h2cb.append(h2cb);
                                    for in_cb in range(1, 21):
                                        n = 0.1 * in_cb;
                                        ana_pi0.fix_cb_n(n);
                                        for ialpha_cb in range(1, 101):
                                            alpha = 0.5 * ialpha_cb;
                                            ana_pi0.fix_cb_alpha(alpha);
                                            outlist_int_range = ana_pi0.analyze_ptspectrum();
                                            outlist_int_range.SetName("integral_{0:2.1f}_{1:2.1f}_sigma".format(integral_sigma_min, integral_sigma_max));
                                            #outlist_fit_range.Add(outlist_int_range);
                                            #outlist_int_range.ls();
                                            for ipt in range(0, len(arr_pt)-1):
                                                f1sig = outlist_int_range.FindObject("f1sig_pt{0:d}".format(ipt));
                                                chi2_per_ndf = f1sig.GetChisquare()/f1sig.GetNDF();
                                                list_h2cb[ipt].Fill(n, alpha, chi2_per_ndf);
                                    for ipt in range(0, len(arr_pt)-1):
                                        list_h2cb[ipt].SetContour(100);
                                        outfile.WriteTObject(list_h2cb[ipt]);
                outfile.WriteTObject(outlist_ss);
                outlist_ss.Clear();
            del ana_pi0;
        outfile.Close();
